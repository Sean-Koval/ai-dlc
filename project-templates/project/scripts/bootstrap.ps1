...
$nativeArchitecture = [Microsoft.Win32.Registry]::GetValue('HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Environment', 'PROCESSOR_ARCHITECTURE', $null)
if ([Environment]::OSVersion.Platform -ne [PlatformID]::Win32NT -or -not [Environment]::Is64BitProcess -or $nativeArchitecture -ne 'AMD64') {
...
$stagePath = $null
$priorProjectEnvironment = [Environment]::GetEnvironmentVariable('UV_PROJECT_ENVIRONMENT', 'Process')
$priorPath = $env:PATH
$completed = $false
try {
    $rootGuard = [AiDlc.Bootstrap.DirectoryGuard]::new($Root, $false, $false); $guards.Add($rootGuard)
    $assetGuard = [AiDlc.Bootstrap.DirectoryGuard]::new((Join-Path $Root 'bootstrap'), $false, $false); $guards.Add($assetGuard)
    ...
    $readers.Add($pythonGuard.ProtectForExecution())
    $managedPython = Open-ManagedPython $env:UV_PYTHON_INSTALL_DIR $pins.python_version
    if ($null -eq $managedPython) {
        Invoke-NativeTool $uv @('--no-config','python','install','--managed-python',$pins.python_version)
        $managedPython = Open-ManagedPython $env:UV_PYTHON_INSTALL_DIR $pins.python_version
    }
    if ($null -eq $managedPython) { throw 'Verified managed Python is unavailable.' }
    $guards.Add($managedPython.Guard); $readers.Add($managedPython.Reader)
    $python = $managedPython.Executable
    Invoke-NativeTool $python @('--version')
    ...
    if ($Source) {
        $revision = Get-SourceProvenance $Root
        $inputText = $Root + $pins.python_version + $revision
        ...
        try {
            ...
            if ($prior.input_sha256 -eq $inputHash -and [IO.Path]::GetDirectoryName($prior.environment) -eq $homeDirectory -and [IO.Path]::GetFileName($prior.environment).StartsWith($prefix + '-')) {
                ...
                try {
                    ...
                    try {
                        $candidateReader = Open-VerifiedArtifact $candidateScripts 'ai-dlc.exe' $prior.launcher_sha256
                        & $candidate --version | Out-Host
                        if ($LASTEXITCODE -eq 0) {
                            $environmentPath = $prior.environment; $reuse = $true
                            $guards.Add($candidateGuard); $guards.Add($candidateScripts); $readers.Add($candidateReader)
                        }
                    } finally { if (-not $reuse) { if ($candidateReader) { $candidateReader.Dispose() }; $candidateScripts.Dispose() } }
                } finally { if (-not $reuse) { $candidateGuard.Dispose() } }
            }
    ...
    $readers.Add($environmentGuard.ProtectForExecution())
    if ($Source -and -not $reuse) {
        ...
        $readers.Add((Open-VerifiedArtifact $downloads $manifest.AI_DLC_WHEEL_NAME $manifest.AI_DLC_WHEEL_SHA256)); $readers.Add((Open-VerifiedArtifact $downloads $constraintsName $manifest.AI_DLC_CONSTRAINTS_SHA256))
        Invoke-NativeTool $uv @('--no-config','venv','--allow-existing','--python',$python,$environmentPath)
    ...
    [Environment]::SetEnvironmentVariable('UV_PROJECT_ENVIRONMENT', $priorProjectEnvironment, 'Process')
    ...
    [Environment]::SetEnvironmentVariable('UV_PROJECT_ENVIRONMENT', $priorProjectEnvironment, 'Process')
    if (-not $completed) { $env:PATH = $priorPath }
