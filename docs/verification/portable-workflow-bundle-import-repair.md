...
Independent re-review accepted `fb8a4c8` with no actionable findings and closed
the recovery-source substitution P1. The accepted renderer retention code stayed
unchanged through that repair. Integration with the native harness child uses
its shared client directory registry and bound render reads for native rules/MCP.
Two new filesystem integration cases failed before fixing the old bundle-only
